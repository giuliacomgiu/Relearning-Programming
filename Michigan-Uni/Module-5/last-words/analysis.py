...
w=[]
c=[]
many = input('How many most used words would you like to see? ')
try:
	many = int(many)
except:
	many = input('Please, digits only: ')
many = int(many)
...
word = input('Type in a word to see how many times it was used: ').lower()
if word.isalpha() == False and word.isdigit() == False:
	word = input('Please only characters or numbers, not both or anything else:').lower()
cur.execute('''SELECT word, count  
	FROM Words WHERE word = ?''',(word,))
wc = cur.fetchone()
print(wc[0],'was used',wc[1],'times')